package com.koolearn.cloud.util.numberToChinese;

import org.apache.commons.lang.StringUtils;

/**
 * Created by gehaisong on 2016/5/8.
 */
public class ParseRmbNumberToChinese {
    public static void main(String[] args) {
        System.out.println(parseRmbNumToChinese("00006640000000044.28448888"));
    }
     ...
     * @param moneyNum String
     * @return 转换好的中文金额大写形式的字符串
     */
        private static String parseRmbNumToChinese(String moneyNum) {
            moneyNum=roundString(moneyNum);//进行四舍五入处理
            // 如果传入的是空串则继续返回空串
                    if(StringUtils.isBlank(moneyNum)) {  return"";  }
            // 以小数点为界分割这个字符串
                    int index = moneyNum.indexOf(".");
            // 截取并转换这个数的整数部分
                    String intOnly = moneyNum.substring(0, index);
                    String part1 =numFormat(1, intOnly);
            // 截取并转换这个数的小数部分
                    String smallOnly = moneyNum.substring(index +1);
                    String part2 =numFormat(2, smallOnly);
            // 把转换好了的整数部分和小数部分重新拼凑一个新的字符串
                    String newS = part1 + part2;
               newS= cleanZero(newS);//去零
                    return newS;
        }
    /**
