...
<script type="text/javascript" src="<%=request.getContextPath()%>/js/essay.js"></script>
	...
	function save(){
		...
		if(checkForm()){
			...
			$("input[name^='essayQuestionDTOs']").each(function(){
				...
				if(lengthLarger(answer.val(), 255)){
					err1.show();
					isReturn = true;
					$(this).focus();
					alertMsg("答案长度不能超过255字符!");	
					return;
				}
			});
			if(answerBool){
...
function detail(){
	...
	if(checkForm()){
		...
		if(!isReturn){
			setRadioValue();
			windowOpen(context_path+'/maintain/essay/detail/0?questionType=<%=Question.QUESTION_TYPE_FILL_BLANK%>'+"&1="+Math.random(),780,400);
...
<body>
<div class="path"><a href="/pages/index.jsp">首页</a><c:if test="${not empty sessionScope.userTemplateName}"><span>></span>${sessionScope.userTemplateName}</c:if><span>></span><a href="/pages/menu.jsp">试题管理</a><span>></span><em>手动录入试题_普通填空</em></div>
<form action="/maintain/essay/insert" name="form1" method="post">
<jsp:include page="hiddenessay2.jsp" flush="true"/> 
