...
<body>
<form action="<%=basePath%>question/base/spokenQuestion/modify" method="post" name="spokenForm" id="spokenForm">
<input type="hidden" name="spokenQuestion.id" value="${dto.spokenQuestion.id }"/>
<input type="hidden" name="spokenQuestion.questionId" value="${dto.spokenQuestion.questionId }"/>
<input type="hidden" name="questionDto.question.id" value="${dto.questionDto.question.id }"/>
...
<c:if test="${not empty(dto.questionDto) && not empty(dto.questionDto.responseType )}">
	<input type="hidden" name="questionDto.responseType" value="${dto.questionDto.responseType }"/>
</c:if>
...
<input type="hidden" name="questionDto.questionBankExt.tag1" value="${dto.questionDto.questionBankExt.tag1 }"/> 
<input type="hidden" name="questionDto.questionBankExt.tag2" value="${dto.questionDto.questionBankExt.tag2 }"/>
<input type="hidden" name="questionDto.questionBankExt.tag3" value="${dto.questionDto.questionBankExt.tag3 }"/> 
<input type="hidden" name="questionDto.questionBankExt.status" value="${dto.questionDto.questionBankExt.status }"/>
...
<div class="wp1">
	...
	<div class="fml" style="display:none;">
		...
		<input type="text" name="spokenQuestion.answer" value="${dto.spokenQuestion.answer }"  maxLength="255" class="ipt"/><a href="javascript:;" class="a1 bjk">编辑框</a>
	...
	<div class="fmt">
		<div class="fmbx fmbx2  fc">
			...
			<textarea class="ipta fck">${dto.spokenQuestion.topic}</textarea>
			<span class="error1">题干不能为空</span>
			<span class="error1">题干不能超过65535个字符</span>
			<div class="ta_r">
				<a href="javascript:;" class="btn1 addtg">增加题干</a>
	...
	<div class="ontabtxt">
		...
		<textarea id="spokenQuestion.scorestandad"  name="spokenQuestion.scorestandad"  class="ipta" style="display: none;">${dto.spokenQuestion.scorestandad}</textarea>
		<script type="text/javascript">
	...
	<textarea id="spokenQuestion.answerreference" name="spokenQuestion.answerreference"  class="ipta" style="display: none;">${dto.spokenQuestion.answerreference}</textarea>
	<script type="text/javascript">
	...
	<div class="fml">
		...
		<input type="text" name="spokenQuestion.recordtime" id="spokenQuestion.recordtime" value="${dto.spokenQuestion.recordtime }"  class="ipt"/>分钟（为空表示不限时）<span class="error1">高度必须为大于0的整数</span>
	</div>
	...
	<div class="wp1">
	<div class="ta_c pd1">
		<a href="javascript:;" class="btn1" onclick="saveSpoken(${saveType});return false;">完&nbsp;&nbsp;成</a>
