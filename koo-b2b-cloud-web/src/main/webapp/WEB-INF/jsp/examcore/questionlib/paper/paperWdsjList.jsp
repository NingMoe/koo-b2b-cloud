    ...
    <c:forEach items="${paperList}" var="pa"  varStatus="status">
        <tr paperId="${pa.id}">
            <td class="fst-pd fst-td">
                        <span class="sp-pd">
                             <a title="${pa.paperName}" href="/teacher/exam/core/paper/preview/?paperId=${pa.id}&createFrom=1&navigation=<%=GlobalConstant.PAPER_CREATE_TYPE_wdsj%>" paperId="${pa.id}">${pa.paperName}</a>
                        </span>
            </td>
