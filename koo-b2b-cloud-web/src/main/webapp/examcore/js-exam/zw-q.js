...
function zwInit(){
	...
	$(".ZwEdit").bind("mouseup",function(e){
		...
		if ((rangy.getSelection().getRangeAt(0)+"")!="" && surroundRange()) {
			...
			if(wp){
				setBZ.css({"left":bzleft,"top":xy.y});
			}else{
				//zzz
				setBZ.css({"left":bzleft+120,"top":xy.y}).find(".bzticon").css({"left":xy.x-(bzleft/2)});
			}
			EmptySelection();
		}
...
function editBZ(o){//点击修改链接，修改相应标注
	...
	xy.left+=bzi.width()/2;
	//zzz
	setBZ.css({"left":xy.left-setBZ.width()/2+60,"top":xy.top});
	clearBZform();
