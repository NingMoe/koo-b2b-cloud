...
function isPaperView(){
	if(window.location.href.indexOf("/paper/preview/")!=-1){
		return true;
	}
	return false;
}
$(function(){
	...
	$(".ZwEdit").bind("mouseup",function(e){
		...
		$(this).parents(".ep_details").find(".bztit2 .btn1_b").click();
		if ((rangy.getSelection().getRangeAt(0)+"")!="" && surroundRange()) {
			var setBZ = $(this).parents(".ep_details").find(".bztit2").show();
			xy = $(this).children('#ZWBZ_hover').offset();
			xyz= $(this).parents(".ep_details").offset();
			var divwidth = 120;
			setBZ.css({"left":xy.left-xyz.left-divwidth,"top":xy.top-xyz.top});
			EmptySelection();
		}
	...
	$(".bztit2").find(".btn1_b").click(function(){//添加修改标注中的取消链接
...
function editBZ(o){//点击修改链接，修改相应标注
	var bzi = $(o).closest(".bzi"),ZWBZ=bzi.prev(".ZWBZ"),setBZ = $(o).parents(".ep_details").find(".bztit2").show(),xy = bzi.offset();
	//xy.left+=bzi.width()/2;
	//setBZ.css({"left":xy.left-setBZ.width()/2,"top":xy.top});
	var xy = bzi.offset();
	var xyz =$(o).closest('.ep_details').offset();
	var divWidth = 120;
	setBZ.css({"left":xy.left-xyz.left-divWidth,"top":xy.top-xyz.top});
	clearBZform();
...
function addBZ(o){//添加修改一个标注
	var ZWBZ = $("#ZWBZ_hover"), radio = $(".bztit2 input[name='radioBZ']"),title,id,
	editipt = $(o).parents(".ep_details").find(".bztit2 .editipt").focus().val();
	content="";
	var input = ZWBZ.closest("dt").find(".sc_ans_html");
	var answer = ZWBZ.closest("dt").find(".sc_ans");
	radio.each(function(){
	...
	$(".bztit2 .editipt").val("");
	//$(this).parents(".ep_details").find(".bztit2 .editipt").val("");
	content = ZWBZ.text();
	if(title=="修改"){
