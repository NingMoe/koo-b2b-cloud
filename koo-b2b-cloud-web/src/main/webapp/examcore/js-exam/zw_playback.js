...
$(function(){
	...
	$(".ZwEdit").bind("mouseup",function(e){
		...
		if ((rangy.getSelection().getRangeAt(0)+"")!="" && surroundRange()) {
			...
			setBZ.css({"left":xy.x-setBZ.width()/2,"top":xy.y});
			EmptySelection();
		}
...
function setBZ(){//解析当前标注元素,this需要为.ZWBZ
	...
	$(this).after($('<span class="bzi"><div class="bztit">\
...
function addBZ(){//添加修改一个标注
	...
	var input = ZWBZ.closest("dt").find("input:hidden");
	radio.each(function(){
	...
	content = ZWBZ.text();
	if(title=="修改"){
		content = '<span class="fred">[</span>' + content +'<span class="fred">]</span>' + " 修改为 " + '<span class="fred">[</span>' + editipt + '<span class="fred">]</span>';
	}else{
		content = "删除 " + '<span class="fred">[</span>' + content +'<span class="fred">]</span>';
	}
	ZWBZ.attr({
		"title":title,
		"content":content,
