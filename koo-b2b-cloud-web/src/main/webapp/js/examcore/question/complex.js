...
//	$(".ontab").ontab(".ontabfn",".ontabtxt","selected","click");
	...
	$("#preview").click(function(){
		...
		if(validate()&&checkChoiceTopic() && checkChoice() && checkCB()){
			...
			$("#complexForm").attr("action","/question/base/complex/preview/1");
			$("#complexForm").submit();
		}
	});
	...
	$("#submit").click(function(){
		...
		fillHeadValue();
		if(validate() &&checkChoiceTopic() && checkChoice() && checkCB()){
			$("#complexForm").attr("action","/question/base/complex/insert");
...
function rsetfm(fm){
	...
	$(".subExp",fm).attr("name","choiceQuestionDtos["+index+"].questionDto.question.explan");
	$("li",fm).each(function(i){
		$(".xh",this).html(String.fromCharCode(65+i));
...
function rsetfms(n){
	...
	var fmbx = $(".fmbx"+n).each(function(i){
		...
		$(".subExp",this).attr("name","choiceQuestionDtos["+i+"].questionDto.question.explan");
	});
...
function editMsg(ipt) {
	...
	if(ipt.nextAll(".fn").size()==0){
		...
		<form id='upfile_form1' action='/question/base/upload2/optImg' method='POST' enctype='multipart/form-data' target='screct_frame' class='ta_c mb10' >添加图片：<input type='file' name='imagefile1' class='ml1' /><input type='hidden' name='randomNum' value='"+n+"'><a href='javascript:;' id='uploadImage' class='btn1 ml1'>上传</a></form>\
		<form id='upfile_form2' action='/question/base/upload2/optMp3' method='POST' enctype='multipart/form-data' target='screct_frame' class='ta_c mb10' >添加音频：<input type='file' name='mp3file2' class='ml1' /><input type='hidden' name='randomNum' value='"+n+"'><a href='javascript:;' id='uploadMp3' class='btn1 ml1'>上传</a></form>\
...
function fckEditMsg(ipt,path) {
	...
	rc_pop.setContent("alertCon", "<textarea class='ipta2' id='fckMsg' name='fckMsg'></textarea><div class='ta_c'><a href='javascript:;' class='btn1'>完成</a></div>");
	rc_pop.setContent("title", "编辑文本");
	rc_pop.build();
	rc_pop.show();
	rsPopup(rc_pop);
	
	var oFCKeditor = new FCKeditor('fckMsg');
    oFCKeditor.BasePath = path+"/js/fckeditor/";
	...
	$("#fckMsg").data("ipt",ipt).val(ipt.val()).next().find(".btn1").click(function(){
		//var area = $(this).closest("div").prev();
		//area.data("ipt").val(area.val());
		if(rc_pop.okCallBack){
			var oEditor=FCKeditorAPI.GetInstance("fckMsg");
			var content=oEditor.GetXHTML(true);
			rc_pop.okCallBack(content);
		}
		rc_pop.close();
	}).parent().parent().ontab(".btn2","form","sd","click").find("form").hide();
	return rc_pop;
	...
	tkbox = html.find(".tkbox").each(function(i){
		...
		if($(this).hasClass("newtkbox")){
			...
			if (bhxx) {
				...
				li = $('<li index="' + newindex + '"><span class="tktit">(' + (eq + 1) + ')</span><input type="text" class="ipt2" name="essayQuestionDTOs['+eq+'].fillblankAnswers[0].answer"/></li>');
				if (i) {
					lis.eq(i - 1).after(li);
				}
				else {
...
function checkChoice(){
	...
	$(".fmbx1").find("input[type='text']").each(function(){
		...
		if(le){
			...
			$(this).focus().siblings(".error1").html("答案长度不能大于"+len+"个字符").show();
		}
		result = le && result;
	});
	return result;
