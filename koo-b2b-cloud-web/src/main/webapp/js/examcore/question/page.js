...
$(".addtg").live("click",function(){
	var fm = addfm2($(this).closest(".fmbx2")), callback = $(this).attr("addCallBack");
	if(callback){
		window[callback](fm);
	}
	$(".fmbx2").find('.fmfn').show();
	//addfm();
});

$(".addword").live("click",function(){
	var fm = addfm3($(this).closest(".fmbx3")), callback = $(this).attr("addCallBack");
	if(callback){
		window[callback](fm);
	}
	//addfm();
});
$(".adddl").live("click",function(){
	adddl($(this).closest(".dl"));
});
...
$(".dl .fmclose").live("click",function(){
	$(".dl").size()>1 && $(this).closest(".dl").remove();
	rsetdls();
	return false;
});
$(".fmbx2 .fmclose").live("click",function(){
	...
	if($(".fmbx2").find('.fmfn').size()==1){
		$(".fmbx2").find('.fmfn:eq(0)').hide();
	}
	return false;
});
...
function adddl(dl){
	dl = dl || $(".dl:last");
	var fmt = dl.closest(".fmt"), m = fmt.attr("m") || 101;
	var ndl = $(".dl:eq(0)").removeClass("fmone").clone();
	dl.after(ndl);
	ndl.find(".ipt").val("");
	rsetdls();
	fmt.attr("m",m);
...
function commentWin(link,tit) {
	...
	if(!sumScore || parseInt(sumScore) == 0){
		if(tit == "模块"){
			popAlert("模块总分为0，请先添加大题和试题!",null,270);
		}else{
	...
	for (var i = 0; i < len || i == 0; i++) {
		...
		html += '<div class="fmbx fmbx2 unfmone"><form class="commentForm"><div class="fc">\
				...
				<div class="ta_r">\
					<a href="javascript:;" onclick="commentAdvanced(this);return false;" class="a1 fl">高级</a>\
	...
	$(".pyTxt .addpy").die().live("click",function(){
		var checkScore = document.getElementById("sumScore") ;
		var es = $(this).parent().prev().prev().find("input[name='endScore']").val();
		var error = $(this).parent().siblings(".error1");
		if(checkScore && parseFloat(es) == parseFloat(sumScore)){
			error.text("分数段已达到最高分，无法继续添加").show();
			return false;
		}else{
			error.empty().hide();
			if(commValidate()){
...
function addtr(ele,edit,page){
	...
	var arg='dialogWidth:'+($(top).width()*0.9)+'px;DialogHeight='+($(top).height()*0.9)+'px;fullscreen:yes;maximize:yes;dialogTop:0px;dialogLeft:0;help=0;center=1;status:yes;scroll=1';
	if(typeof(showModalDialog)=='function'){
		...
		arg='height='+($(top).height()*0.9)+'px,width='+($(top).width()*0.9)+'px,top=0,left=0,toolbar=no,menubar=no,dependent=yes,scrollbars=yes, resizable=yes,location=no, status=no';
...
$('.addDanci').live('click', function(){
	var f = $(this).closest('.zhaoci'), ipt = $('.key', f), list = $('.gjclist', f), key = $.trim(ipt.val()), hd = $("input:hidden", f);
	if(key==ipt.attr("df") || key=='' || key.len()>50 || (new RegExp("(^|;)"+key+",").test(hd.val())))return;
	list.append('<dd><span class="liTag">'+key+'<span class="close3" title="删除"></span></span>&nbsp;个数：<input type="number" min="1" value="1" name="word_num" reg="^([1-9]\d*)?$" class="ipt ipt3 word_num"/><span class="error"></span></dd>');
	updtgjc2(list,hd);
	ipt.val("");
});

$(".zhaoci .gjclist .ipt3").live("blur",function(){
	var f = $(this).closest('.zhaoci'), ipt = $('.ipt', f), list = $('.gjclist', f), key = $.trim(ipt.val()), hd = $("input:hidden", f);
	updtgjc2(list,hd);
});

$(".zhaoci .close3").live("click",function(){
	var f = $(this).closest('.zhaoci'), ipt = $('.ipt', f), list = $('.gjclist', f), key = $.trim(ipt.val()), hd = $("input:hidden", f);
	$(this).closest("dd").remove();
	updtgjc2(list,hd);
});
$(".fmbx3 .fmclose").live("click",function(){
	if ($(".fmbx3").size() > 1) {
		$(this).closest(".fmbx3").remove();
		$(".fmbx3").each(function(i){
			$("input:hidden", $(this)).attr("name","lookupWordAnswers["+i+"].target");
			$(".ipta", $(this)).attr("name","lookupWordAnswers["+i+"].alternative");
		});
	}
	return false;
});
//更新关键词
function updtgjc2(dl,hd){
	var s = "";
	$("dd",dl).each(function(){
		s+=$(".liTag", this).text()+"#"+$(".ipt3", this).val()+"|";
	});
	hd.val($.trim(s));
}
//添加题干后的回调函数，用于初始化结构，同时修改了19行。
function zhaoCiAddCallBack(fm){
	$(".key", fm).val(function(){return $(this).attr("df");});
	$("input:hidden", fm).val("");
	$(".gjclist", fm).empty();
...
function addfm3(fmbx3){
	fmbx3 = fmbx3 || $(".fmbx3:last");
	var nfm = $(".fmbx3:eq(0)").removeClass("fmone").clone();
	fmbx3.after(nfm);
	nfm.find(".ipta").val("");
	rsetfms(3);
	return nfm;
...
//插入一个位置
function addwz3(ipta,fm,name){
ordwz3(ipta,fm,name);
}
//重新给位置排序
function ordwz3(ipta,fm,name){
	var html = $("<div />"), tb1, ul = fm.find("ul"), li;
	html.html(ipta.html());
	tb1 = html.find(".tb4");
	ul.addClass("hide2").children("li:gt(0)").remove();
	tb1.each(function(i){
		...
		li.find(".fjct").html(v).next("input[name='hidden']").val(v);
		ul.removeClass("hide2");
	});
	rsetfms(1);
...
//评语编辑框变为fck编辑器
function commentAdvanced(o){
	var obj=$(o);
	var t=obj.parent().prev().prev().find(".ipta");
	t.attr("id", "_ipta_temp_").hide();
	var oFCKeditor = new FCKeditor('_ipta_temp_');
    ...
    obj.parent().children().hide().parent().append($('<a href="javascript:;" class="btn1 mr10">完成</a>').click(function(){
    	var oEditor=FCKeditorAPI.GetInstance("_ipta_temp_");
		var content=oEditor.GetXHTML(true);
		t.val(content).show().removeAttr("id").prevAll("iframe,input").remove();
		obj.parent().children().show().eq(-1).remove();
    }));
}
//试卷导出
function exportPaper(paperId){
	confirmMsg("导出试卷到word吗,导出过程耗时较长请耐心等待！",exportPaperSubmit);
    function exportPaperSubmit(){
    	document.exportPaperForm.paperId.value = paperId;
    	document.exportPaperForm.submit();
	}
}
