			...
			if(true){
				jQuery(document).ready(function() {
					...
					jQuery.getScript(conf.jsDirectory+'FlexPaperViewer.js', function() {
						...
						/*
						 * @cuidingfeng
						 * 修改搜索没匹配项时alert的样式和内容
						 */
						var _searchText2_ =  window[instance].searchText, _searchText_ = function(a,b){
							var _alert_ = window.alert, myAlert = function(s){
								if(!s)s="没有匹配项或搜索结束！";
								if(s==='Finishedsearching')s="搜索完毕！";
								if(s==='No more search matches.')s="没有更多的搜索结果！";
								if(!window.alertMsg)alertMsg = _alert_;
								alertMsg(s);
							};
							if(a===''){
								alertMsg("请输入搜索词！");
							...
							_searchText2_.call(window[instance],a,b);
							window.alert = _alert_;
						};
						window[instance]['searchText'] = window[instance].searchText = _searchText_;
						/*edit end*/
